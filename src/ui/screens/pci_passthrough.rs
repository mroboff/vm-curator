...
pub fn render(app: &App, frame: &mut Frame) {
    ...
    let help_text = if app.config.enable_multi_gpu_passthrough && !app.config.single_gpu_enabled {
        "[Space] Toggle  [g] Auto-select GPU  [a] Add IOMMU siblings  [Enter/s] Save  [p] Prereqs  [Esc] Back"
    } else {
        "[Space] Toggle  [a] Add IOMMU siblings  [Enter/s] Save  [Esc] Back"
    };
...
pub fn handle_key(app: &mut App, key: crossterm::event::KeyEvent) -> anyhow::Result<()> {
    ...
    match key.code {
        ...
        KeyCode::Char('s') | KeyCode::Char('S') | KeyCode::Enter => {
            // Pre-save viability check: if any selected device's IOMMU group has

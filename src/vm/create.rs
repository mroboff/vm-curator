...
/// Configuration for GPU passthrough script
#[allow(dead_code)]
#[derive(Debug, Clone)]
pub struct GpuPassthroughConfig {
    /// GPU PCI address (e.g., "01:00.0")
    pub gpu_address: String,
    /// Optional audio device PCI address (e.g., "01:00.1")
    pub audio_address: Option<String>,
    /// IVSHMEM size in MB (for Looking Glass)
    ...
    /// Whether to enable UEFI
    pub uefi: bool,
}

/// Generate a GPU passthrough launch script with Looking Glass support
#[allow(dead_code)]
pub fn generate_gpu_passthrough_script(config: &GpuPassthroughConfig) -> String {
    let mut script = String::new();

    // Shebang and header
    script.push_str("#!/bin/bash\n");
    script.push_str("# GPU Passthrough Launch Script\n");
    script.push_str(&format!("# {} - GPU passthrough with Looking Glass\n", config.vm_name));
    script.push_str("# Generated by vm-curator\n");
    script.push_str("#\n");
    script.push_str("# Prerequisites:\n");
    script.push_str("#   - IOMMU enabled (intel_iommu=on or amd_iommu=on in kernel params)\n");
    script.push_str("#   - vfio-pci modules loaded (modprobe vfio-pci)\n");
    script.push_str("#   - GPU bound to vfio-pci driver\n");
    script.push_str("#   - Looking Glass Host installed in guest (for display)\n");
    script.push_str("#   - HDMI/DP dummy plug connected to passthrough GPU\n");
    script.push_str("\n");

    // Check if running as root (needed for VFIO)
    script.push_str("# Check for VFIO permissions\n");
    script.push_str("if [[ ! -r /dev/vfio/vfio ]]; then\n");
    script.push_str("    echo \"Error: Cannot read /dev/vfio/vfio\"\n");
    script.push_str("    echo \"Make sure VFIO modules are loaded and you have permission\"\n");
    script.push_str("    exit 1\n");
    script.push_str("fi\n\n");
    ...
    script.push_str(&format!("GPU_ADDR=\"{}\"\n", config.gpu_address));
    if let Some(ref audio_addr) = config.audio_address {
        script.push_str(&format!("AUDIO_ADDR=\"{}\"\n", audio_addr));
    }
    script.push_str(&format!("IVSHMEM_SIZE=\"{}M\"\n", config.ivshmem_size_mb));
    script.push_str("IVSHMEM_PATH=\"/dev/shm/looking-glass\"\n");
    script.push('\n');

    // Setup IVSHMEM for Looking Glass
    script.push_str("# Setup IVSHMEM for Looking Glass\n");
    script.push_str("setup_ivshmem() {\n");
    script.push_str("    if [[ ! -e \"$IVSHMEM_PATH\" ]]; then\n");
    script.push_str("        touch \"$IVSHMEM_PATH\"\n");
    script.push_str("    fi\n");
    script.push_str("    # Set appropriate permissions\n");
    script.push_str("    chmod 660 \"$IVSHMEM_PATH\" 2>/dev/null || true\n");
    script.push_str("    # Try to set group to kvm if available\n");
    script.push_str("    chown \"$USER:kvm\" \"$IVSHMEM_PATH\" 2>/dev/null || true\n");
    script.push_str("}\n\n");
    ...
    // Build QEMU command
    script.push_str("echo \"Launching with GPU passthrough...\"\n");
    script.push_str("echo \"GPU: $GPU_ADDR\"\n");
    script.push_str("echo \"Looking Glass IVSHMEM: $IVSHMEM_PATH ($IVSHMEM_SIZE)\"\n");
    script.push_str("echo \"\"\n\n");

    script.push_str("exec qemu-system-x86_64 \\\n");
    script.push_str("    -enable-kvm \\\n");
    script.push_str("    -machine q35,accel=kvm \\\n");

    // CPU
    if let Some(ref cpu_model) = config.cpu_model {
        script.push_str(&format!("    -cpu {} \\\n", cpu_model));
    } else {
        script.push_str("    -cpu host \\\n");
    }
    script.push_str(&format!("    -smp {},sockets=1,cores={},threads=1 \\\n", config.cpu_cores, config.cpu_cores));
    ...
    if config.uefi {
        let ovmf_path = find_ovmf_code_path()
            .unwrap_or_else(|| "/usr/share/OVMF/OVMF_CODE.fd".to_string());
        script.push_str(&format!("    -drive if=pflash,format=raw,readonly=on,file={} \\\n", ovmf_path));
    }

    // Disk
    script.push_str("    -drive file=\"$DISK\",format=qcow2,if=virtio \\\n");

    // GPU passthrough with VFIO
    script.push_str("    -device vfio-pci,host=$GPU_ADDR,multifunction=on,x-vga=on \\\n");

    // Audio passthrough if configured
    if config.audio_address.is_some() {
        script.push_str("    -device vfio-pci,host=$AUDIO_ADDR \\\n");
    }

    // IVSHMEM for Looking Glass
    script.push_str("    -device ivshmem-plain,memdev=ivshmem,bus=pcie.0 \\\n");
    script.push_str("    -object memory-backend-file,id=ivshmem,share=on,mem-path=\"$IVSHMEM_PATH\",size=\"$IVSHMEM_SIZE\" \\\n");

    // No display on host (Looking Glass handles it)
    script.push_str("    -vga none \\\n");
    ...
    // USB for input passthrough
    script.push_str("    -usb \\\n");
    script.push_str("    -device usb-tablet \\\n");

    // SPICE for fallback/setup (can be removed after Looking Glass is working)
    script.push_str("    -spice port=5900,disable-ticketing=on\n");

    script
}

/// Write the GPU passthrough script to disk
#[allow(dead_code)]
pub fn write_gpu_passthrough_script(vm_dir: &Path, content: &str) -> Result<PathBuf> {
    use std::os::unix::fs::PermissionsExt;

    let script_path = vm_dir.join("launch-with-gpu-passthrough.sh");

    fs::write(&script_path, content)
        .with_context(|| format!("Failed to write GPU passthrough script: {}", script_path.display()))?;
...
/// Remove the GPU passthrough script from a VM
#[allow(dead_code)]
pub fn remove_gpu_passthrough_script(vm_dir: &Path) -> Result<()> {
    let script_path = vm_dir.join("launch-with-gpu-passthrough.sh");

    if script_path.exists() {
        fs::remove_file(&script_path)
            .with_context(|| format!("Failed to remove GPU passthrough script: {}", script_path.display()))?;
    }
